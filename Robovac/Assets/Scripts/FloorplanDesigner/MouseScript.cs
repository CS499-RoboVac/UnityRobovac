...
public class MouseScript : MonoBehaviour
{
    ...
    public enum ItemList { Cylinder, Cube, Sphere, Vaccum }; // the list of items
    ...
    void CreateObject()
    {
        ...
        else if (itemOption == ItemList.Vaccum) // player start
        {
            if (ms.playerPlaced == false) // only perform next actions if player not yet placed.
            {
                ...
                newObj = Instantiate(Vaccum, transform.position, Quaternion.identity);
                newObj.layer = 9; // set to Spawned Objects layer
