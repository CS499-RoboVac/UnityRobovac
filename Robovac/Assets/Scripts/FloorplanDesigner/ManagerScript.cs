...
public class ManagerScript : MonoBehaviour
{
    ...
    {
        user.itemOption = MouseScript.ItemList.Vaccum; // set object to place as Vacuum
        mouseObject.mesh = playerMarker; // set mouse object's mesh to Vacuum mesh
    }
    ...
    void CreateFromFile()
    {
        ...
        for (int i = 0; i < level.editorObjects.Count; i++)
        {
            ...
            else if (level.editorObjects[i].objectType == EditorObject.ObjectType.Player)
            {
                ...
                newObj = Instantiate(user.Vaccum, transform.position, Quaternion.identity);
                newObj.layer = 9; // assign to SpawnedObjects layer.
